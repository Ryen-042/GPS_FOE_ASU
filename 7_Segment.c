...
void PORTABD_Init(void){
	...
	GPIO_PORTD_CR_R |= 0xCF; // Allow Changes to PortD
	GPIO_PORTD_AMSEL_R &= ~0xCF; // Disable Analog Pins 
	GPIO_PORTD_PCTL_R &= ~0x00FFFFFF; // Disable Serial Pin-Type For Pins
	GPIO_PORTD_AFSEL_R &= ~0xCF; // Disable Serial Pins
	GPIO_PORTD_DIR_R |= 0xCF; // Set Pins as Output
	GPIO_PORTD_DEN_R |= 0xCF; // Set Pins as Digital
	...
	GPIO_PORTE_AMSEL_R &= ~0x36; // Disable Analog Pins 
	GPIO_PORTE_PCTL_R &= ~0x00FF0FF0; // Disable Serial Pin-Type For Pins
	GPIO_PORTE_AFSEL_R &= ~0x36; // Disable Serial Pins
	GPIO_PORTE_DIR_R |= 0x36; // Set Pins as Output
	GPIO_PORTE_DEN_R |= 0x36; // Set Pins as Digital
}
