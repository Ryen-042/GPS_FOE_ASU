...
void PORTABD_Init(void){
	...
	GPIO_PORTA_CR_R |= 0xFC; // Allow Changes to PortA
	GPIO_PORTA_AMSEL_R &= ~0xFC; // Disable Analog Pins 
	GPIO_PORTA_PCTL_R &= ~0xFFFFFF00; // Disable Serial Pin-Type For Pins
	GPIO_PORTA_AFSEL_R &= ~0xFC; // Disable Serial Pins
	GPIO_PORTA_DIR_R |= 0xFC; // Set Pins as Output
	GPIO_PORTA_DEN_R |= 0xFC; // Set Pins as Digital
	...
	GPIO_PORTD_CR_R |= 0xCC; // Allow Changes to PortD
	GPIO_PORTD_AMSEL_R &= ~0xCC; // Disable Analog Pins 
	GPIO_PORTD_PCTL_R &= ~0xFFFFFFFF; // Disable Serial Pin-Type For Pins
	GPIO_PORTD_AFSEL_R &= ~0xCC; // Disable Serial Pins
	GPIO_PORTD_DIR_R |= 0xCC; // Set Pins (D2 D3 D6 D7)as Output
	GPIO_PORTD_DEN_R |= 0xCC; // Set Pins as Digital
	//GPIO_PORTD_DATA_R &= 0x00; // Initalize Pins -> All Pins Are Active (For Active Low 7-Segment)
	
	// port (E)
	GPIO_PORTE_LOCK_R = 0x4C4F434B;
	GPIO_PORTE_CR_R = 0x3F; // Allow Changes to PortE (0, 1, 2, 4, 5, 6)
	GPIO_PORTE_AMSEL_R &= ~0x3F; // Disable Analog Pins 
	GPIO_PORTE_PCTL_R &= ~0x00FFFFFF; // Disable Serial Pin-Type For Pins
	GPIO_PORTE_AFSEL_R &= ~0x3F; // Disable Serial Pins
	GPIO_PORTE_DIR_R |= 0x3F; // Set Pins as Output
	GPIO_PORTE_DEN_R |= 0x3F; // Set Pins as Digital
}
...
unsigned int PORTD_in_E(unsigned int c){
	...
	// Missing Pins in PortD -> (0, 1, 4, 5 )
	switch (c) {
		// We will not shift Pins (0, 4, 5) when we Put them in PortE and will Put Them in the same Pins (i.e. Pins (0, 4, 5) of PortE)
			// but we need to shift pin 1 to pin 6 in port E 
		case 0:
		d = 0x00;    // 1000 1000 (Before Shifting) 
		break;

		case 1:
		d = 0x38;    // 1011 1110 (Before Shifting)
		break;

		case 2:
		d = 0x11;    // 0001 1001 (Before Shifting)
		break;
		...
		case 4:
		d = 0x28;    // 0010 1110 (Before Shifting)
		break;
